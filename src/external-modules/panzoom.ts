         ...
         1: [
            function (require, module, exports) {
               ...
               function createPanZoom(domElement, options) {
                  ...
                  function centerOnElement(ele) {
                     const body = document.body.getBoundingClientRect()
                     const parentRect = owner.getBoundingClientRect()
                     ...
                     const transformX = parentCenterX - elementCenterX
                     const transformY = parentCenterY - elementCenterY

                     internalMoveBy(transformX, transformY, true)
                  }

                  function leftJustify(ele, smooth) {
                     const body = document.body.getBoundingClientRect()
                     const parentRect = owner.getBoundingClientRect()

                     const elementRect = ele.getBoundingClientRect()

                     const parentCenterY = body.top + body.height / 2
                     const elementCenterY = elementRect.top + elementRect.height / 2

                     const transformX = parentRect.left - elementRect.left
                     const transformY = parentCenterY - elementCenterY

                     internalMoveBy(transformX, transformY, smooth !== false)
                  }
                  ...
                  function zoomByRatio(clientX, clientY, ratio) {
                     ...
                     // stops the weird shifting to the left when zoom
                  ...
                  function newZoom(ratio) {
                     var ownerRect = owner.getBoundingClientRect()
                     var tOrigin = {
                        // x: ownerRect.width * transformOrigin.x - 430,
                        x: ownerRect.width * transformOrigin.x,
                        y: ownerRect.height * transformOrigin.y,
                     }

                     transform.x = tOrigin.x - ratio * (tOrigin.x - transform.x)
                     transform.y = tOrigin.y - ratio * (tOrigin.y - transform.y)
