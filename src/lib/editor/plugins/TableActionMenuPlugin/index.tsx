...
function TableActionMenu({
            ...
            for (let i = 0; i < nodes.length; i++) {
               ...
               if (DEPRECATED_$isGridCellNode(node)) {
                  if (isFirstCell) {
                     ...
                     invariant(lastDescendant !== null, "Unexpected empty lastDescendant on the resulting merged cell")
   ...
   return createPortal(
      ...
      >
         {cellMerge && (selectionCounts.columns > 1 || selectionCounts.rows > 1) && canMergeCells && (
            <>
               <button className="item" onClick={() => mergeTableColumnsAtSelection()} data-test-id="table-merge-cells">
                  Merge cells
               </button>
               <hr />
            </>
